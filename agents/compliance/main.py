...
def main() -> None:
    ...
    # Skill tools default to approval_mode="always_require": the runtime emits an
    # mcp_approval_request for every load_skill call and pauses for a caller
    # approval that our unattended backend never sends, so response.output_text
    # comes back empty ("Agent returned empty output"). Opt the read-only skill
    # tools out of approval so they run unattended (never_require). No scripts are
    # bundled, so run_skill_script approval is left at its default.
